...
def _init_session_state():
    if "inspection_log" not in st.session_state:
        # Try to load existing CSV on first run (local mode only)
...
3. The AI will process each image and annotate any defects it finds.
4. Review results in the **Analytics Dashboard** that appears after processing.
...
if uploaded_files and model:
    ...
    for idx, file in enumerate(uploaded_files):
        ...
        current_img_placeholder.image(img_rgb, caption=f"Inspecting: {safe_name}", use_container_width=True)
        ...
        if detected_labels:
            ...
            counts = Counter()
            summary_text = ""
            current_info_placeholder.info(f"Unit {safe_name} is clean.")
    ...
    # Batch summary
    render_batch_summary(batch_results, skipped_files)
